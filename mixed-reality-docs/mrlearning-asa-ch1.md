...
[Asset Pack del modulo ASA v 1.3.1](https://github.com/Developer-OI/MixedRealityLearning/releases/download/ASA_1.3/ASAModuleAssets_1.3.1.unitypackage)
...
6. Importare l' [Asset Pack del modulo di base di Mr](https://github.com/microsoft/MixedRealityLearning/releases/tag/1.2) avanti. Analogamente al passaggio 5, tornare a Unity, fare clic su assets e passare il puntatore del mouse su Import Package. Fare clic su pacchetto personalizzato... I file del computer verranno visualizzati di nuovo. Passare alla posizione in cui è stato archiviato il modulo di base asset Pack. e selezionarlo. Fare clic su Apri.
...
7. Importare il [modulo ASA Pack 1.3.1](https://github.com/Developer-OI/MixedRealityLearning/releases/download/ASA_1.3/ASAModuleAssets_1.3.1.unitypackage) usando la stessa procedura usata per importare gli altri pacchetti precedenti.
...
10. Per eseguire lo zoom avanti sulla scena, fare doppio clic sull'ancoraggio padre nella gerarchia e modificare la visualizzazione per visualizzare l'intera scena in base alle esigenze. Attualmente, ParentAnchor è un cubo colorato utilizzato solo per scopi dimostrativi. Infine, il cubo verrà nascosto e il contenuto verrà inserito come elemento figlio di ParentAnchor. 

11. Ora è possibile configurare i pulsanti per il funzionamento della scena. A tale proposito, espandere la prefabbricata ButtonParent e notare diversi pulsanti con etichetta. Questi pulsanti vengono creati dai prefabbricati PressableButton di MRTK. Altre informazioni su come creare PressableButton dal modulo di [base](mrlearning-base-ch2.md). Per far funzionare questi pulsanti, è necessario aggiungere un evento che verrà attivato quando l'utente preme o seleziona i singoli pulsanti. 

- Per il pulsante denominato StartAzureSession, creare un nuovo evento sotto il trigger di evento premuto pulsante, nonché il trigger di evento clic su. Trascinare l'oggetto ParentAnchor nel campo vuoto e assegnare il metodo StartAzureSession () dal componente ASAmoduleScript dell'oggetto ParentAnchor, come illustrato nella schermata seguente.
...
- Per il pulsante denominato, avviare la ricerca dell'ancoraggio, creare un nuovo evento sotto il pulsante premere "trigger evento" e il trigger evento clic. Trascinare l'oggetto ParentAnchor nel campo vuoto e assegnare il metodo FindAzureAnchor () dal componente ASAmoduleScript dell'oggetto ParentAnchor.
...
8. Anche dopo una compilazione corretta, è possibile che si verifichino errori, come illustrato di seguito, ma se la compilazione ha esito positivo, è possibile ignorarli e procedere con i passaggi successivi.
...
9. Al termine della compilazione, apri la nuova cartella creata che contiene i file dell'applicazione appena compilata. Fare doppio clic sulla soluzione "MixedRealityBase. sln" o sul nome corrispondente. Se è stato usato un nome alternativo per il progetto per aprire il file della soluzione in Visual Studio.
    ...
    > Nota: se Visual Studio chiede di installare nuovi componenti, verificare che tutti i componenti dei prerequisiti siano installati come specifici nella [pagina "installazione degli strumenti"](install-the-tools.md) .
...
1. Cercare il prefabbricato "lanciarazzi complete" e trascinarlo nella gerarchia come figlio dell'oggetto, come illustrato nell'immagine seguente.
