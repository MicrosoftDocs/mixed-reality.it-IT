...
|----------|----------|----------|
|  Attivato  |  Nessuna  |  Mostra la schermata iniziale di Unity predefinita per 5 secondi o fino a quando non viene caricata l'app, a seconda del valore più lungo | 
...
|  Off  |  Nessuna  |  Mostra il nero trasparente (Nothing) finché l'app non viene caricata. | 
...
Per altre informazioni, vedere [la documentazione della schermata iniziale](https://docs.unity3d.com/Manual/class-PlayerSettingsSplashScreen.html) di Unity.
...
Se si verifica una perdita di rilevamento, il comportamento predefinito di Unity prevede l'arresto del rendering degli ologrammi, la sospensione del [ciclo di gioco](http://docs.unity3d.com/Manual/ExecutionOrder.html)e la visualizzazione di una notifica di rilevamento perso che segue comodamente lo sguardo degli utenti. Le notifiche personalizzate possono essere fornite anche sotto forma di immagine di perdita del rilevamento. Per le app che dipendono dal rilevamento per l'intera esperienza, è sufficiente consentire a Unity di gestire completamente questa operazione fino a quando il rilevamento non viene recuperato. Gli sviluppatori possono fornire un'immagine personalizzata da visualizzare durante la perdita del rilevamento. 
